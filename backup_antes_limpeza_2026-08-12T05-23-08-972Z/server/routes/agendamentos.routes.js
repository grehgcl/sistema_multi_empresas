...
// ============================================
// GET /api/agendamentos - CORRIGIDO
// ============================================
...
router.post('/', auth, async (req, res) => {
    ...
    // 🔥 EXECUTAR O INSERT NO BANCO DA EMPRESA
    ...
    async function finalizarResposta(agendamentoId) {
        ...
        try {
            ...
            if (cliente && empresa && cliente.telefone) {
                ...
                console.log(`✅ WhatsApp: Confirmação enviada`);
            }
        } catch (whatsappError) {
            console.error('❌ Erro ao enviar WhatsApp:', whatsappError.message);
        }

        // 🔥 RETORNAR O ID CORRETAMENTE
        res.json({
            success: true,
            data: {
                id: agendamentoId,
...
// ============================================
// DELETE /api/agendamentos/:id - CORRIGIDO
// ============================================
router.delete('/:id', auth, (req, res) => {
    try {
        ...
        empresaDb.get(sqlCheck, [id, empresaId], (err, agendamento) => {
            ...
            if (!agendamento) {
                console.log(`⚠️ Agendamento ${id} não encontrado`);
                return res.status(404).json({
                    success: false,
                    message: 'Agendamento não encontrado'
                });
...
// ============================================
// PUT /api/agendamentos/:id/pagamento - CONCLUIR COM PAGAMENTO
// ============================================
