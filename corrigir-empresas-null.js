// server/routes/auth.routes.js - VERSÃO COMPLETA CORRIGIDA
...
// ============================================
// POST /api/auth/cadastro - CORRIGIDO
// ============================================
router.post('/cadastro', async (req, res) => {
    ...
    try {
        ...
        // ============================================
        // 3. VERIFICAR EMPRESA (COM RETRY)
        // ============================================
        console.log(`🔍 Verificando empresa ${empresaId}...`);

        let empresaVerificada = null;
        let tentativas = 0;
        const maxTentativas = 10;

        while (tentativas < maxTentativas && !empresaVerificada) {
            tentativas++;
            ...
            await new Promise(resolve => {
                db.get('SELECT id, nome FROM empresas WHERE id = ?', [empresaId], (err, row) => {
                    if (err) {
                        ...
                        console.log(`   ✅ Empresa encontrada na tentativa ${tentativas}`);
                        empresaVerificada = row;
                        resolve();
                    } else {
            ...
            if (!empresaVerificada && tentativas < maxTentativas) {
        ...
        if (!empresaVerificada) {
            console.log('   ⚠️ Buscando pelo nome...');
            empresaVerificada = await new Promise((resolve) => {
                db.get('SELECT id, nome FROM empresas WHERE nome = ? ORDER BY id DESC LIMIT 1', [empresa_nome], (err, row) => {
                    if (err) {
            ...
            if (empresaVerificada) {
                console.log(`   ✅ Empresa encontrada pelo nome! ID: ${empresaVerificada.id}`);
                empresaId = Number(empresaVerificada.id);
            }
        }

        // Último recurso: listar todas
        if (!empresaVerificada) {
            console.log('   ⚠️ Listando todas as empresas...');
            const todas = await new Promise((resolve) => {
                db.all('SELECT id, nome FROM empresas ORDER BY id DESC LIMIT 10', (err, rows) => {
                    if (err) {
                        console.error('❌ Erro:', err);
                        resolve([]);
                    } else {
            ...
            console.log('   📋 Últimas empresas criadas:');
            todas.forEach(e => console.log(`      ID: ${e.id} - Nome: ${e.nome}`));
            
            const encontrada = todas.find(e => e.nome === empresa_nome);
            if (encontrada && encontrada.id) {
                console.log(`   ✅ Empresa encontrada na lista! ID: ${encontrada.id}`);
                empresaVerificada = encontrada;
                empresaId = Number(encontrada.id);
            }
        }

        if (!empresaVerificada) {
            throw new Error(`Empresa não encontrada! ID: ${empresaId}, Nome: ${empresa_nome}`);
        }
        ...
        // 5. Criar tabelas (código completo aqui...)
        await new Promise((resolve, reject) => {
            empresaDb.serialize(() => {
