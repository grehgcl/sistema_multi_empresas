...
async function migrar() {
  try {
    ...
    if (empresaDestino.rows.length === 0) {
      ...
      await destino.query(
        'INSERT INTO empresas (nome, plano, limite_profissionais, trial_expira) VALUES ($1, $2, $3, $4)',
        ['Studio Sandro matias', 'pro', 5, new Date(Date.now() + 45 * 24 * 60 * 60 * 1000)]
      );
      ...
      empresaId = empresaDestino.rows[0].id;
        ...
        if (existe.rows.length === 0) {
          await destino.query(`
            INSERT INTO agendamentos (
              ...
              empresa_id, profissional_id, duracao
