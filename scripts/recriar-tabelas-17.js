...
console.log(`📁 Recriando tabelas no banco: empresa_${empresaId}.db`);
...
// Criar todas as tabelas (usando IF NOT EXISTS)
empresaDb.serialize(() => {
    ...
    empresaDb.run(`CREATE TABLE IF NOT EXISTS clientes (
        ...
        if (err) console.error('❌ Erro clientes:', err.message);
    ...
    empresaDb.run(`CREATE TABLE IF NOT EXISTS servicos (
        ...
        if (err) console.error('❌ Erro servicos:', err.message);
    ...
    empresaDb.run(`CREATE TABLE IF NOT EXISTS profissionais (
        ...
        if (err) console.error('❌ Erro profissionais:', err.message);
        else console.log('   ✅ profissionais');
    });
    ...
    empresaDb.run(`CREATE TABLE IF NOT EXISTS agendamentos (
        ...
        if (err) console.error('❌ Erro agendamentos:', err.message);
        else console.log('   ✅ agendamentos');
    });
    ...
    empresaDb.run(`CREATE TABLE IF NOT EXISTS despesas (
        ...
        if (err) console.error('❌ Erro despesas:', err.message);
        else console.log('   ✅ despesas');
    });
    ...
    empresaDb.run(`CREATE TABLE IF NOT EXISTS horarios_funcionamento (
        ...
        if (err) console.error('❌ Erro horarios:', err.message);
        else console.log('   ✅ horarios_funcionamento');
    });
    ...
    empresaDb.run(`CREATE TABLE IF NOT EXISTS configuracoes (
        ...
        if (err) console.error('❌ Erro configuracoes:', err.message);
        else console.log('   ✅ configuracoes');
    });

    console.log('📋 Inserindo dados padrão (se não existirem)...');
    ...
    console.log('✅ Banco empresa_17.db verificado e atualizado!');
